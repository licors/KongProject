    ...
    <section class="cgt-content-wrapper main-content clear-fix">
        <div class="cgt_support" style="padding-top:50px; max-width:1200px; margin:0px auto;">
            ...
            <div style="width:100%; text-align:center;font-weight:bold;border-bottom:2px solid #d1d1d1;" class="clearfix">
                ...
                <div style="float: left; width: 13%; padding: 20px 0px;">유형</div>
                <div style="float: left; width: 62%; padding: 20px 0px;">내용</div>
                <div style="float: left; width: 10%; padding: 20px 0px;">답변여부</div>
                <div style="float: left; width: 10%; padding: 20px 0px;">작성일</div>
            </div>
            <c:forEach var="list" items="${list}" varStatus="stat">
                <c:if test="${list.re_level eq 0}">
                    <a href="/support/view/${list.support_num}" class="btn_cgt_notice_detail cgt-single-load">
                        <div style="width: 100%; text-align: center; border-bottom: 1px solid #d1d1d1;" class="clearfix">
                            ...
                            <div style="float: left; width: 13%; padding: 20px 0px;">${list.type}</div>
                            <div style="float: left; width: 62%; padding: 20px 0px;text-align:left;"><c:if test="${list.re_level ne 0}"><c:forEach var = "i" begin = "${list.re_level}" end = "0">&nbsp;</c:forEach>→</c:if><c:choose><c:when test="${fn:length(list.content) > 50}"><c:out value="${fn:substring(list.content,0,49)}"/>...</c:when><c:otherwise><c:out value="${list.content}"/></c:otherwise></c:choose></div>
                                    <div style="float: left; width: 10%; padding: 20px 0px;text-align:center;"><div class="reply"></div>
                                        <script type="text/javascript">
                                            $(function () {
                                                jQuery.ajax({
                                                    url: "/support/reply/" + ${list.support_num},
                                                    type: "get",
                                                    success: function () {
                                                        jQuery('.reply').text('답변완료');
                                                    },
                                                    error: function () {
                                                        jQuery('.reply').text('답변중');
                                                    }
                                                });
                                            });
                                </script>
